...
import { getOcultos, leerPapelera } from "./papelera";
...
export async function getViajes() {
  ...
  const [listasPorRaiz, { ocultos }] = await Promise.all([
    Promise.all(
      raices.map(async (rootId) => {
        const archivos = await listarSubcarpetas(rootId, apiKey);
        return archivos.map((f) => ({ ...f, raiz: rootId }));
      })
    ),
    getOcultos(),
  ]);

  const todos = listasPorRaiz.flat();
